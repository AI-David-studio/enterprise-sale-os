...
export function InviteClient({ token, invite, currentUser }: InviteClientProps) {
  ...
  const roleLabel = ROLE_LABELS[invite.roleName] || invite.roleName

  // Check email match for authenticated users
  const emailMatch =
    currentUser &&
  ...
  async function handleAcceptExisting() {
    ...
    try {
      const result = await acceptInviteExistingAccount(token)
      if (result.success) {
        setSuccess(true)
  ...
  async function handleAcceptNewAccount(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await acceptInviteWithNewAccount(token, password, firstName, lastName)
      if (result.success) {
        setSuccess(true)
        setTimeout(() => router.push('/dashboard'), 1500)
      } else {
    ...
    return (
      <div className="flex-1 flex flex-col w-full px-8 sm:max-w-lg justify-center pt-20 mx-auto">
        <div className="bg-green-50 border border-green-200 rounded-lg p-6 text-center">
          ...
          <p className="text-green-700">Перенаправляем в панель управления…</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex-1 flex flex-col w-full px-8 sm:max-w-lg justify-center pt-12 mx-auto">
      ...
      <div className="bg-white border rounded-lg p-6 mb-6 shadow-sm">
        <h1 className="text-2xl font-semibold mb-4">Приглашение в сделку</h1>

        <dl className="space-y-2 text-sm">
          <div className="flex gap-2">
            <dt className="font-medium text-gray-500 w-32 shrink-0">Сделка:</dt>
            <dd className="text-gray-900">{invite.dealName}</dd>
          </div>
          <div className="flex gap-2">
            <dt className="font-medium text-gray-500 w-32 shrink-0">Роль:</dt>
            <dd className="text-gray-900">{roleLabel}</dd>
          </div>
          <div className="flex gap-2">
            <dt className="font-medium text-gray-500 w-32 shrink-0">Приглашает:</dt>
            <dd className="text-gray-900">{invite.inviterName}</dd>
          </div>
          <div className="flex gap-2">
            <dt className="font-medium text-gray-500 w-32 shrink-0">Email:</dt>
            <dd className="text-gray-900 font-mono">{invite.email}</dd>
          </div>
        </dl>
      </div>
      ...
      {currentUser && !emailMatch && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
          <p className="text-sm text-yellow-800">
            Вы вошли как <strong>{currentUser.email}</strong>, но приглашение предназначено для <strong>{invite.email}</strong>.
          </p>
          <p className="text-sm text-yellow-700 mt-2">
            Выйдите из текущего аккаунта и войдите или зарегистрируйтесь с адресом <strong>{invite.email}</strong>.
          </p>
        </div>
      )}

      {/* PATH C: Unauthenticated user — two options */}
      {!currentUser && (
        <>
