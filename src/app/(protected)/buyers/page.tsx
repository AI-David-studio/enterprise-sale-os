...
export default async function BuyersPage() {
  ...
  if (activeDealId) {
    try {
      const role = await getUserDealRole(user.id, activeDealId)
      isLeadAdvisor = role === 'lead_advisor'
    } catch {
      // Role check failed — hide create controls
    }
  }
  ...
  if (activeDealId) {
    const { data } = await supabase
      .from('buyers')
      .select('id, name, industry, website, buyer_pipeline_states(pipeline_stages(name))')
        ...
        <table className="w-full text-left">
          <thead className="bg-gray-50 border-b">
            <tr>
              ...
              <th className="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Этап</th>
              <th className="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Действия</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-200">
            {buyers.map((buyer) => {
              const stageName = (buyer.buyer_pipeline_states?.[0]?.pipeline_stages as any)?.name || '—'
              return (
            ...
            {buyers.length === 0 && (
              <tr>
                <td colSpan={4} className="px-6 py-12 text-center text-gray-500">
                  Нет активных покупателей. {isLeadAdvisor ? 'Нажмите «+ Добавить покупателя», чтобы начать работу.' : ''}
                </td>
              </tr>
            )}
          </tbody>
