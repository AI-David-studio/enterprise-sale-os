...
import { ExternalToggleButton } from './external-toggle-button'
...
export default async function DocumentsPage() {
  ...
  // Resolve active deal context — all deal members can view documents
  const ctx = await getCurrentUserActiveDealContext(user.id)
  if (!ctx?.dealId) {
    return redirect('/dashboard')
  }

  const canUpload = ctx.roleName === 'lead_advisor' || ctx.roleName === 'advisor'
  const canToggleExternal = ctx.roleName === 'lead_advisor'

  // Fetch documents for the active deal
            ...
            {documents.map((doc) => (
              <tr key={doc.id} className="hover:bg-gray-50">
                <td className="px-6 py-4 font-medium text-gray-900 flex items-center gap-2">
                  ...
                  {doc.name}
                  {doc.is_external && (
                    <span className="ml-2 inline-block px-2 py-0.5 text-xs font-medium rounded-full bg-green-100 text-green-800">
                      Внешний
                    </span>
                  )}
                </td>
                ...
                <td className="px-6 py-4 text-sm flex gap-3">
                  ...
                  {canToggleExternal && (
                    <ExternalToggleButton documentId={doc.id} isExternal={doc.is_external} />
                  )}
                </td>
              </tr>
            ))}
              ...
              <tr>
                <td colSpan={4} className="px-6 py-12 text-center text-gray-500">
                  <div className="flex flex-col items-center justify-center">
                    ...
                    <p>Нет файлов в хранилище. {canUpload ? 'Нажмите «Загрузить файл», чтобы добавить документы.' : 'Документы ещё не загружены.'}</p>
                  </div>
                </td>
              </tr>
            )}
